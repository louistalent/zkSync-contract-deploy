...
contract SPIRALPrivateSale is ReentrancyGuard {
	using SafeMath for uint256;
	...
	// Price of presale SPIRAL, 1 ETH
	
    uint256 public SPIRALPerETH =  3818;
	...
	constructor(
		...
		SPIRAL = IBEP20(_SPIRAL);
    }

    function setSPIRALPerETH(uint256 _SPIRALPerETH) public onlyOwner{
        SPIRALPerETH = _SPIRALPerETH;
    }
	...
	function buy(uint256 _amount) public payable nonReentrant {
		...
		uint256 tokens = _amount.mul(SPIRALPerETH);
        require(msg.value >= _amount, "Influence ETH");
		...
		ethReceived = ethReceived.add(_amount);
	...
	function getSPIRALTokensLeft() external view returns (uint256) {
		return SPIRAL.balanceOf(address(this));
	}

	function claimTokens() external nonReentrant {
		require(isClaimActive, "Claim is not allowed yet");
		require(tokensOwned[msg.sender] > 0, "User should own some SPIRAL tokens");
		require(tokensUnclaimed[msg.sender] > 0, "User should have unclaimed SPIRAL tokens");
		require(SPIRAL.balanceOf(address(this)) >= tokensUnclaimed[msg.sender], "There are not enough SPIRAL tokens to transfer.");

		uint256 callerTokensUnclaimed = tokensUnclaimed[msg.sender];
		tokensUnclaimed[msg.sender] = 0;
		SPIRAL.safeTransfer(msg.sender, callerTokensUnclaimed);
	...
	function withdrawUnsoldSPIRAL() external onlyOwner {
		uint256 amount = SPIRAL.balanceOf(address(this)) - totalTokensSold;
		SPIRAL.safeTransfer(msg.sender, amount);
	}

	function withdrawAllSPIRAL() external onlyOwner {
